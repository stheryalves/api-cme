...
async function calculoVolumeTotalDiarioPorLead(id) {
    ...
    try {
        ...
        tipoProcessamento
        FROM \`lead\` WHERE id = ?`;
        const [results] = await connection.query(query, [id]); // Usa o id passado como argumento

        if (results.length === 0) {
            return null; // Retorna null se não houver resultados para o id dado
        }
        ...
        if (tipoProcessamento == 0) {
            ...
            return {
                ...
                estimativaVolumeTotalDiarioInstrumentalUE: Math.ceil(estimativaVolumeTotalDiarioInstrumentalUE * 2 * 10) / 10,
                estimativaVolumeTotalDiarioInstrumentalLt: Math.ceil(estimativaVolumeTotalDiarioInstrumentalLt * 2 * 10) / 10,
...
async function visualizarResultados() {
    try {
        ...
        for (const id of ids) {
            ...
            const resultadoAllIds = await getAllLeadIds(id);
            resultados.push(resultado);
            //resultados.push(resultadoAllIds);
        }
